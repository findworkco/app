...
scenario.route('A request to POST /add-application/received-offer (specific)', {
  ...
  scenario.routeTest('for a logged in user and valid form data', function () {
    ...
    httpUtils.session.init().login()
      .save(serverUtils.getUrl('/add-application/received-offer'))
      .save({
  ...
  scenario.routeTest('for a logged in user and invalid form data', function () {
    ...
    it('reuses submitted values in inputs/textareas', function () {
      ...
      expect(this.$('input[name=received_offer_reminder_enabled][value=yes]').attr('checked'))
        .to.equal('checked');
      expect(this.$('input[name=received_offer_reminder_enabled][value=no]').attr('checked'))
        .to.equal(undefined);
      expect(this.$('input[name=received_offer_reminder_date]').val()).to.equal('2016-01-01');
