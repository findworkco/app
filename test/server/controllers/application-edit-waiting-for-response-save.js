...
scenario.route('A request to POST /application/:id (waiting for response)', {
  ...
  scenario.routeTest('for a logged in user and valid form data', {
    ...
    sinonUtils.spy(Application.Instance.prototype, 'updateOrReplaceWaitingForResponseReminder');
    ...
    it('runs `Application.updateOrReplaceWaitingForResponseReminder()`', function () {
      // DEV: This verifies reminders are replaced when appropriate as tested in model tests
      var updateOrReplaceWaitingForResponseReminderSpy =
        Application.Instance.prototype.updateOrReplaceWaitingForResponseReminder;
      expect(updateOrReplaceWaitingForResponseReminderSpy.callCount).to.equal(1);
    });
