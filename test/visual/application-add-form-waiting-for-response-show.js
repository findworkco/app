...
gemini.suite('application-add-form-waiting-for-response-show', function (suite) {
  ...
  suite.load('/add-application/waiting-for-response', geminiUtils.SETUPS.DEFAULT)
    ...
    .before(function normalizeRelativeValues (actions, find) {
      ...
      actions.executeJS(function handleExecuteJS (window) {
        ...
        window.document.querySelector('[name=follow_up_reminder_date]').value = '2016-05-20';
        window.document.querySelector('[name=follow_up_reminder_time]').value = '7:00PM';
      });
    })
