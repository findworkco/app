...
gemini.suite('application-add-form-received-offer-show', function (suite) {
  ...
  gemini.suite('default', function (child) {
    child.load('/add-application/received-offer', geminiUtils.SETUPS.DEFAULT)
      ...
      .before(function normalizeRelativeValues (actions, find) {
        ...
        actions.executeJS(function handleExecuteJS (window) {
          ...
          window.document.querySelector('[name=received_offer_reminder_date]').value = '2022-05-20';
  ...
  gemini.suite('invalid', function (child) {
    child.load('/add-application/received-offer', geminiUtils.SETUPS.DEFAULT)
      ...
      .before(function submitForm (actions, find) {
        actions.click('form[action="/add-application/received-offer"] button[type=submit]');
      })
      .capture('default-large', geminiUtils.resizeLarge)
      .capture('default-medium', geminiUtils.resizeMedium)
      .capture('default-small', geminiUtils.resizeSmall);
